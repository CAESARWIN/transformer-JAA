...
def make_dataset(image_list, land, biocular, type):
    len_ = len(image_list)
    images = [(image_list[i].strip(), land[i, :], biocular[i], type[i, :]) for i in range(len_)]

    return images

def make_dataset1(image_list, type):
    len_ = len(image_list)
    images = [(image_list[i].strip(), type[i:]) for i in range(len_)]
...
class ImageList1(object):
    ...
    def __init__(self, crop_size, path, phase='train', transform=None, target_transform=None,
        ...
        # land = np.loadtxt(path + '_land.txt')
        # biocular = np.loadtxt(path + '_biocular.txt')
        emotion = np.loadtxt(path + '_emotion.txt')
        imgs = make_dataset1(image_list, emotion)
        if len(imgs) == 0:
            raise (RuntimeError('Found 0 images in subfolders of: ' + path + '\n'))
