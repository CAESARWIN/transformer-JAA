...
def AU_detection_evalv2(loader, region_learning, align_net, local_attention_refine,
                local_au_net, global_au_feat, au_net, gcn, L_sym, use_gpu=True, fail_threshold = 0.1):
    missing_label = 9
    test_batch_nb = len(loader)

    for i, batch in enumerate(loader):
        if i % 100 == 0:
            print('batch[%d/%d]' % (i, test_batch_nb))
        input, au = batch
        if use_gpu:
        ...
        concat_au_feat = torch.cat((concat_au_feat.view(concat_au_feat.size(0), -1), gcn_feature), 1)
    ...
    np.savetxt('val_predAUprob_all_.txt', AUoccur_pred_prob, fmt='%f',
               delimiter='\t')
    # AUs
    AUoccur_pred = np.zeros(AUoccur_pred_prob.shape)
    AUoccur_pred[AUoccur_pred_prob < 0.5] = 0
    ...
    return local_f1score_arr, local_acc_arr, f1score_arr, acc_arr
