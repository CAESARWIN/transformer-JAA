        ...
        for i in range(align_output.size(0)):
            ...
            # elif self.au_num == 8:
                ...
                generate_map(aus_map[i, 10], self.crop_size, self.map_size + 8, self.spatial_ratio, self.fill_coeff,
                             arr2d[0, 34], arr2d[1, 34], arr2d[0, 40], arr2d[1, 40])
                # au26
                generate_map(aus_map[i, 11], self.crop_size, self.map_size + 8, self.spatial_ratio, self.fill_coeff,
                             arr2d[0, 39], arr2d[1, 39] + ruler / 2, arr2d[0, 41], arr2d[1, 41] + ruler / 2)
...
class LocalAUNetv2(nn.Module):
    ...
    def forward(self, feat, aus_map):
        ...
        for i in range(len(self.local_aus_branch)):
            ...
            au_feat = feat * au_map
            output_au_feat = self.local_aus_branch[i](au_feat)
            reshape_output_au_feat = output_au_feat.contiguous().view(output_au_feat.size(0), -1)
            ...
            if i == 0:
                ...
                # 各个AU特征
                local_aus_feats = reshape_output_au_feat.unsqueeze(1)
            else:
                aus_feat = aus_feat + output_au_feat
                aus_output = torch.cat((aus_output, au_output), 2)

                local_aus_feats = torch.cat((local_aus_feats, reshape_output_au_feat.unsqueeze(1)), 1)
        # average over all AUs
        aus_feat = aus_feat / float(len(self.local_aus_branch))
        return aus_feat, aus_output, local_aus_feats
...
class GraphConvolution_for_AU(nn.Module):
    def __init__(self, in_features, out_features, bias=False):
        super(GraphConvolution_for_AU, self).__init__()
        self.in_features = in_features
        self.out_features = out_features
        self.weight = nn.Parameter(torch.cuda.FloatTensor(in_features, out_features))
        if bias:
    ...
    def forward(self, input, adj):
        N, M, F = input.size()
        support = torch.mm(input.view(N*M, F), self.weight)
        support = support.view(N, M, self.out_features)
        support = support.permute(1, 0, 2).contiguous()
        support = support.view(M, N*self.out_features)
...
class GCN(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout):
        ...
        self.gc1 = GraphConvolution_for_AU(nfeat, nhid)
        self.gc2 = GraphConvolution_for_AU(nhid, nclass)
        self.dropout = dropout
        self.fc1 = nn.Linear(7 * nclass, 12)
